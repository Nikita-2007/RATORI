...
class Ground(object):
    ...
    def update(self, size, turn, g):
        ...
        # Ограничение крайв
        if self.point_x < size[0] // 2 + scroll_line:
            self.point_x = size[0] // 2 + scroll_line
        if self.point_x > self.max_x - size[0] // 2 - scroll_line:
            self.point_x = self.max_x - size[0] // 2 - scroll_line
        if self.point_y < size[1] // 2 + scroll_line:
            self.point_y = size[1] // 2 + scroll_line
        if self.point_y > self.max_y - size[1] // 2 - scroll_line:
            self.point_y = self.max_y - size[1] // 2 - scroll_line
        self.select()
